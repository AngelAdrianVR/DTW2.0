...
const form = useForm({
    ...
    regimen_fiscal: props.client.regimen_fiscal || 'persona_fisica',
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <form @submit.prevent="submit">
                    <Card class="dark:bg-zinc-900 dark:border-zinc-800 border border-gray-100 shadow-sm rounded-2xl">
                        <template #title>
                            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-zinc-100">Editar Cliente</h2>
                        </template>
                        <template #subtitle>
                            <p class="text-sm sm:text-base text-gray-500 dark:text-zinc-500">Actualiza la información del cliente en el sistema.</p>
                        </template>
                        ...
                        <template #footer>
                            ...
                            <div class="flex flex-col-reverse sm:flex-row justify-end gap-3 mt-4 sm:mt-6 pt-4 border-t border-gray-100 dark:border-zinc-800">
                                <Link :href="route('clients.index')" class="w-full sm:w-auto block">
                                    <Button label="Cancelar" severity="secondary" text class="w-full" />
                                </Link>
                                <Button label="Actualizar Cliente" icon="pi pi-check" class="w-full sm:w-auto !text-[var(--primary-text-color)]" @click="submit" :loading="form.processing" />
                            </div>
                        </template>
                    </Card>
                </form>
